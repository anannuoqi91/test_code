...
def visualize_gmm_coverage(gmm_models, output_image=None):
    ...
    ax1.plot(means[:, 0], means[:, 1], 'b-', alpha=0.3, linewidth=1, label='Trajectory')
...
def print_summary_statistics(gmm_models):
    ...
    print(f"Converged models: {sum(converged)} ({sum(converged)/len(converged)*100:.1f}%)")
    print(f"\nMean positions:")
    print(f"  X: min={means[:, 0].min():.3f}, max={means[:, 0].max():.3f}, avg={means[:, 0].mean():.3f}")
    print(f"  Y: min={means[:, 1].min():.3f}, max={means[:, 1].max():.3f}, avg={means[:, 1].mean():.3f}")
    print(f"\nPoints per model:")
    print(f"  min={min(n_points)}, max={max(n_points)}, avg={np.mean(n_points):.1f}")
...
if __name__ == "__main__":
    # Configuration
    data_dir = "./data/multi_frame_signle_ped/4/"
    output_json = os.path.join(data_dir, "gmm_models.json")  # Save in same directory as PCD files
    output_image = os.path.join(data_dir, "gmm_coverage.png")  # Save in same directory as PCD files
    covariance_type = 'diag'  # Options: 'full', 'diag', 'spherical'
    
    # Train GMM models
    gmm_models = train_gmm_batch(data_dir, output_json, covariance_type)
